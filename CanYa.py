...
class CanYa(BaseModul.BaseModul):

    NAME                = "CanYa"    # Name
    SYMBOL              = "CAN"             # Token Symbol
    CROWDSALE_ADDRESS   = '0x'              # Contract of Crowdsale, leave '0x' if no separate contract was used
    ERC20_ADDRESS       = '0x1d462414fe14cf489c7a21cac78509f4bf8cd7c0'              # Address of ERC20-Contract 
    ...
    def __init__(self, tx):
        ...
        if self.item:
            ...
            if self.item["eth_value"] is not None:
                self.item["eth_value"] = self.item["eth_value"] * self.CAN_TO_ETH
            else:
                self.item["eth_value"] = -1

            if self.item["erc20_value"] is not None:
                self.item["erc20_value"] = self.item["erc20_value"] * self.CAN_TO_ETH
            else:
